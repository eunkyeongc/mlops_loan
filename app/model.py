...
# -> 학습 데이터와 인코더는 데이터 분석가/데이터 기획자가 다루기 쉬운 한글 컬럼명으로 만들어졌다. 두 상황을 잇는 번역 사전 역할을 한다.
FIELD_TO_COLUMN = {
    "age": "나이",
    "gender": "성별",
    "annual_income": "연소득",
...
class LoanModel:
    ...
    대출 모델 아키텍처와 추론(예측) 규칙을 한 객체로 관리한다.
      ...
      -> 운영 추론이 학습 전처리와 정확히 같아야 하는 MLOps의 핵심 원칙을 보여주는 구조이다.
    
    학습 때 쓴 전처리(인코딩, 스케일링)와 서빙 때 쓰는 전처리가 어긋나면 Training-Serving Skew가 발생해 학습 성능과 실서비스 성능이 크게 벌어진다.
    ( 이 클래스가 pipeline, label_encoders, feature_names까지 로드하는 이유, 학습 시 저장했던 전처리 도구를 그대로 재사용해야 두 단계의 결과가 일치한다.)
    """

    def __init__(self):
        # 생성 직후에는 아직 적재되지 않은 상태.
        # main.py에서 /health(헬스체크)를 할 때, self.pipeline이 None이면 초기상태를 의미한다.
        self.pipeline = None
        self.label_encoders: dict[str, Any] = {} # 범주형 데이터를 숫자로 변환하는 인코더(Encoder)를 저장
        self.feature_names: list[str] = []

        self.threshold: float = 0.5     # 임계값(확률을 가지고 승인/거절로 변환하는 기준값)
        # 0.5는 50%보다 높으면 승인이라는 것을 의미한다.
        # 금융권처럼 부실대출(FP)의 비용이 큰 도메인에서는 이 값을 0.6~0.7로 올려 더 보수적으로 심사하기도 한다.

        self.model_version: str = '1.0.0'       # 추론 결과/로그에 모델 비전을 넣을 때 사용하는 메타데이터
        # 예) 재학습된 새 모델을 배포했을 때 예측 로그에 버전을 함께 남기면 어느 버전의 모델이 이 예측을 했는지 추적할 수 있어 
        #     드리프트 분석(주기적으로 조정)이나 문제 발생 시 원인 추적에 도움이 된다.

    def load(self, model_dir: str = 'models') -> None:
        """
        모델, 레이블 인코더, 피처(컬럼) 특성 순서 파일을 디스크에서 불러온다.

        현재 작업폴더가 어디든 동일하게 동작하도록 이 파일의 위치를 기준으로 계산
        Dockerfile의 WORKDIR이 달라지든 이 코드는 항상 이 파일(model.py) 기준으로 models폴더를 찾으므로 실행 위치에 영향을 받지 않는다.
        """
        script_dir = Path(__file__).parent  # app 폴더(현재 파일이 있는 폴더)
        model_path = script_dir.parent / model_dir  # mlops_loan/models
        ...
        if not feature_names_path.exists():
            raise FileNotFoundError(f'레이블 인코더 파일을 찾을 수 없습니다: {feature_names_path}')
    ...
    def _map_to_korean(data: dict[str, Any]) -> dict[str, Any]:
        ...
        #     new_key = FIELD_TO_COLUMN.get(k, k)  # k("age")를 가져와라, k(첫번째)가 없어도 k(두번째)를 가져와라. 에러 발생이 확률이 없음.
        #     result[new_key] = v
        # return result     # return {FIELD_TO_COLUMN.get(k, k) : v for k, v in data.items()}

        for key, value in data.items():
            if key in FIELD_TO_COLUMN:
                korean_key = FIELD_TO_COLUMN[key]
            else:
    ...
    def predict(self, data: dict[str, Any]) -> dict[str, Any]:
        """ 고객 한 명의 입력을 전처리하고 승인 확률과 위험 등급을 계산한다. -> 파이프라인 전체 구현 """
        ...
        # 학습을 진행할 데이터 프레임 생성
        df =  pd.DataFrame([mapped])[self.feature_names]  # 딕셔너릴르 리스트 형태로 묶어줌.
        ...
        # Numpy scaler를 float로 변환해 JSON 직렬화가 가능하게 만든다.
        # model.predict() --> 결과가 0/1
        #model.predict_proba() --> 확률을 줘서 임계값 조정과 A/B/C/D 등급 산정이 가능해진다.
        # [0,1]의 뒤 인덱스 1의 의미 -> 클래스 1(승인)의 확률 열을 의미한다.
        #                           -> 0번 열은 0 : 거절 확률을 의미한다.
...
# 배치 예측용 메서드( 신규 추가 )
# ---------------------------------------------------------------------------------------------------------------------
    def predict_batch(self, data_list: list[dict[str, Any]]) -> list[dict[str, any]]: # [{나이:35}]
        """ 여러 명의 입력을 한 번에 전처리하고, 승인 확률과 위험 등급을 계산한다. """

        if self.pipeline is None:
            raise RuntimeError('모델이 로드됮 않았습니다. load()함수를 먼저 호출하세요!')
        ...
        # predeict_proba를 n행짜리 df에 "한 번만 호출" --> [:,1]로 승인(1) 확률 열만 꺼낸다.
