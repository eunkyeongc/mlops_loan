...
대출 승인 예측 모델을 HTTP API로 제공하는 FastAPI 애플리케이션
    1. 서버 시작시 학습된 모델 아티팩트를 메모리에 적재한다.
    2. 상태 확인용 앤드포인트와 대출 승인 예측 엔드포인트를 노출한다.
...
from contextlib import asynccontextmanager   # 비동기
...
async def lifespan(app: FastAPI):
    ...
    모델을 요청마다 다시 읽지 않고 시작 시 한 번만 로드하므로 예측 지연과 디스크 I/O를 줄일 수 있다.
    ...
    # 여기에 넣어둔 model은 이후 /health, /predict 같은 모든 요청에서 app.state.model로 꺼내 사용할 수 있다. 
    # 모델을 한 번만 로드해서 모든 요청이 같은 모델 인스턴스를 공유하게 만든 부분이다.
    # (전역 변수 대신 app.state를 쓰는 이유는 FastAPI 앱 객체에 속하므로 테스트할 때 앱 인스턴스별로 독립적인 상태를 가질 수 있기 때문)
...
async def health_check():
    ...
    로드벨런서(ALB)가 이 엔드포인트를 주기적으로 호출해 서버 상태를 확인한다. 단순하게 프로세서가 살아있는지만 확인하면 부족하다. 
    웹 서버는 응답하지만, 모델이 로드되지 않아 모든 예측이 실패하는 상태를 놓칠 수 있다. model_loaded 값까지 함께 확인하는 것이 좋다.
    """
...
async def predict(request: LoanRequest):
    """
    검증된 고객정보를 모델에 전달하고 표준응답 스키마로 반환한다.
    FastAPI는 함수 호출 전에 JSON 요청 본문을 LoanRequest로 검증하고, 반환값도 LoanResponse형식에 맞는지 다시 확인한다.
    """

    model = app.state.model

    try:
        # Pydantic 객체를 순수한 dict 로 바꾸어 모델 계층과 API계층을 분리한다.(느슨한 결합)
...
async def predict_batch(request: BatchLoanRequest):
    ...
    try:
        ...
        return BatchLoanResponse(results=[LoanResponse(**r) for r in results])  # '**r' --> r이라는 이름의 딕셔너리
    except RuntimeError as e:
            raise HTTPException(status_code=503, detail=str(e))
...
# ---------------------------------------------------------------------------------------------------------------------
# 모델 정보 엔드포인트 (신규 추가)
# 엔드포인트를 API로 노출하면 좋은 점:
#       - CI/CD 파이프라인에 배포 후 자동 검증 가능
