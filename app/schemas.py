...
여기서 필드 이름과 제약 조건이 흔들리면 model.py의 매핑과 main.py의 엔드포인트 시그니처까지 함께 흔들리므로,
실무에서는 스키마 설계를 가장 먼저하고 리뷰를 거친다.
'''
...
class LoanRequest(BaseModel):
    ...
    ge/le 제약 --> 모델이 학습한 범위 밖의 입력을 API 단계에서 미리 차단
        ex) 나이 200세, 신용점수 9999점 처럼 학습 데이터에 없던 값을 모델에 넣으면 예측 결과를 신뢰할 수 없다.
            Padantic이 이런 값을 모델 호출 전에 422 에러로 걸러주므로 model.py에는 별도의 방어코드가 필요없다.
    """
    age: int = Field(..., 
                     ge=19,     # >= 이상 (gt : > 초과)
                     le=100,    # <= 이하 (lt : < 미만)
...
class LoanResponse(BaseModel):
    ...
    model.py가 돌려주는 dict에 혹시 다른 키가 섞여있어도, 이 스키마에 정의된 필드만 골라서 응답하므로 
    내부 구현 세부사항이 실수로 클라이언트에게 노출되는 것을 막아준다.
    """
    # 임계값을 통과한 최종 승인 여부
    approved: bool = Field(...,
